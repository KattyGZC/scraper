...
def get_data(driver_web,):
    ...
    try:
        file = driver_web.find_elements(By.CLASS_NAME, 'btn btn-primary data-link'.replace(' ','.'))
        file[2].click()
        time.sleep(10)
        return True
    except Exception as e:
...
def data_classification(data):
    try:
        ...
        for region in regions:
            ...
            (data[filter]).to_csv(f'{BASE_DIR}\\scraper_pe\\classified_files\\{region.lower()}.csv')
    except Exception as e:
        return e
